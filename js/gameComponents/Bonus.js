...
const Bonus = ({ number, name, task, isAnswered, answerData, hint, awardTime, expired, secondsToStart, secondsLeft, actualCode, actualBonusCode }) => (
    <View style={[
        styles.mainContainer,
        { backgroundColor:
                (isAnswered && (Helper.isEqualCode(answerData.Answer, actualCode) || Helper.isEqualCode(answerData.Answer, actualBonusCode)))
                    ? Colors.blue
                    : Colors.background,
        }]}
    >
                    ...
                    <View style={{ flexDirection: 'row' }}>
                        ...
                        {(isAnswered && awardTime > 0) && (
                            <Text style={[styles.bonusValue, { color: Colors.bonus }]}>
                                {Helper.formatCount(awardTime, { collapse: true, withUnits: true })}
                            </Text>
                        )}
                    </View>
                </View>
...
export default inject(mapStateToProps)(observer(Bonus));
